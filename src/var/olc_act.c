...
char * rprog_type_to_name ( int );
SCRIPT_DATA *script_lookup( int );
...
const struct olc_help_type help_table[] =
{
    ...
    {	"game",		game_table,	"Juegos de dados."		},
    {	"sex",		sex_table,	"Sexos."			},
    ...
    {	"trapdam",	trapd_flags,	"Tipo de dano de Trampas."	},
    {	"trapeff",	trap_flags,	"Efecto de Trampas."		},
    {	"liquid",	liq_table,	"Tipos de liquidos."		},
    {	"weapon",	attack_table,	"Tipos de Armas."		},
    {   "position",	position_table,	"Posiciones."			},
    {	"mprog",	mprog_flags,	"Tipos de Mob Progs."		},
    {	"apptype",	apply_types,	"Tipos de Applys (2)."		},
    {	"affect2",	affect2_flags,	"Affects extendidos."		},
    ...
    {	"clan",		clan_flags,	"Tipos de clanes."		},
    {	NULL,		NULL,		NULL				}
};
...
/*****************************************************************************
 Name:		show_game_cmds
 Purpose:	Displays settable game functions.
 Called by:	show_help(olc_act.c).
 ****************************************************************************/
void show_game_cmds( CHAR_DATA *ch )
{
    ...
    for (game = 0; game_table[game].function != NULL; game++)
    {
	sprintf( buf, "%-19.18s", &game_table[game].name[5] );
	strcat( buf1, buf );
	if ( ++col % 4 == 0 )
	    strcat( buf1, "\n\r" );
    }
...
bool show_help( CHAR_DATA *ch, char *argument )
{
          ...
          && !str_prefix( arg, help_table[cnt].command ) )
	{
	    ...
	    if ( help_table[cnt].structure == game_table )
	    {
	    	show_game_cmds( ch );
	    	return FALSE;
	    }
...
REDIT( redit_show )
{
    ...
    for ( pexit = pRoom->exits; pexit; pexit = pexit->next )
    {
		...
		capitalize(dir_name[pexit->direccion]),
...
bool change_exit( CHAR_DATA *ch, char *argument, int door )
{
	...
	rev	= rev_dir[door];
	pNExit	= exit_lookup(pToRoom, rev);
	...
	if ( pExit->u1.to_room )
	{
		...
		if ( pNExit )
			free_exit( pToRoom, pNExit );
	}

	/*
	 * Remove this exit.
	 */
	free_exit( pRoom, pExit );
	...
	pExit = exit_lookup( pRoomIndex, rev_dir[door] );
	...
	if ( (pExit = exit_lookup(pRoom, door)) == NULL )
	{
		pExit	= new_exit();
		exit_set(pRoom, pExit);
	}

	pExit->u1.to_room	= get_room_index( value );
	pExit->orig_door	= door;
	pExit->direccion	= door;
...
REDIT( redit_create )
{
    ...
    pRoom->area			= pArea;
    pRoom->vnum			= value;
    pRoom->room_flags		= (IS_SET(pArea->area_flags, AREA_PROTOTIPO) ?
    				ROOM_PROTOTIPO : 0 );
...
REDIT( redit_teleport )
{
    ...
    if (is_number(argument))
       {
          if (get_room_index(atoi( argument )) || (atoi(argument) == 0))
		{
    ...
    send_to_char ( "Syntax : teleport <#xnumber>\n\r", ch);
    return FALSE;
}
...
const struct wear_type wear_table[] =
{
    ...
    {	WEAR_HOLD,	ITEM_HOLD		},
    {   WEAR_LENTES,	ITEM_WEAR_LENTES	},
    {	WEAR_OREJAS,	ITEM_WEAR_OREJAS	},
    {	NO_FLAG,	NO_FLAG			}
};
...
/* OLC		    do_help( ch, "liquids" );    */
	...
	case ITEM_FUMABLE:
	    ...
	    {
	    	default:
	    		do_help( ch, "ITEM_FUMABLE" );
	    		return FALSE;

		case 0:
			pObj->value[0] = URANGE(0, atoi(argument), 100);
			send_to_char( "Numero seteado.\n\r", ch );
			break;
...
OEDIT( oedit_show )
{
    ...
    if ( pObj->clan )
    {
	    sprintf( buf, "Clan:        [%2d] %s\n\r",
	    	pObj->clan,
	    	get_clan_table(pObj->clan)->name );
	    send_to_char( buf, ch );
    }
    ...
    if (IS_TRAP(pObj))
    {
    	sprintf(buf,"Trap effect:	[%s]\n\r",flag_string( trap_flags, pObj->trap->trap_eff ) );
	send_to_char(buf,ch);
	sprintf(buf,"Trap damage:	[%s]\n\r",flag_string( trapd_flags, pObj->trap->trap_dam ) );
	send_to_char(buf,ch);
	sprintf(buf,"Trap charges:	[%d]\n\r",pObj->trap->trap_charge );
	send_to_char(buf,ch);
    }
...
OEDIT( oedit_charges )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char( "Syntax:  charges [number]\n\r", ch );
	return FALSE;
    }
    ...
    pObj->trap->trap_charge = atoi( argument );
...
OEDIT( oedit_trapset)
{
    ...
    if ( IS_TRAP(pObj) )
    {
    	send_to_char( "OEdit : Objeto ya es trampa.\n\r", ch );
    	return FALSE;
    }
...
OEDIT( oedit_trapremove )
{
    ...
    if ( !IS_TRAP(pObj) )
    {
    	send_to_char( "OEdit : Objeto no es trampa.\n\r", ch );
    	return FALSE;
    }
...
OEDIT( oedit_trapeff )
{
	...
	if ( ( value = flag_value( trap_flags, argument ) ) != NO_FLAG )
	{
	    if ( !IS_TRAP(pObj) )
	    	pObj->trap = new_trap();

	    TOGGLE_BIT(pObj->trap->trap_eff, value);

	    send_to_char( "Trap effect toggled.\n\r", ch);
	    return TRUE;
	}
    }

    send_to_char( "Syntax:  trapeff [flag]\n\r"
		  "Type '? trapeff' for a list of flags.\n\r", ch );
    return FALSE;
}
...
OEDIT( oedit_trapdam )
{
	...
	if ( ( value = flag_value( trapd_flags, argument ) ) != NO_FLAG )
	{
	    if ( !IS_TRAP(pObj) )
	    	pObj->trap = new_trap();

	    pObj->trap->trap_dam = 0;
	    SET_BIT(pObj->trap->trap_dam, value);

	    send_to_char( "Trap damage set.\n\r", ch);
	    return TRUE;
	}
    }

    send_to_char( "Syntax:  trapdamage [flag]\n\r"
		  "Type '? trapdam' for a list of flags.\n\r", ch );
    return FALSE;
}
...
OEDIT( oedit_create )
{
    ...
    pObj->vnum			= value;
    pObj->area			= pArea;
    pObj->extra_flags		= (IS_SET(pArea->area_flags, AREA_PROTOTIPO) ?
    					ITEM_PROTOTIPO : 0);
...
MEDIT( medit_show )
{
    ...
    if ( pMob->norecalc )
	add_buf( buffer, "Este mob #BNO#b sera recalculado.\n\r" );
...
/* ROM values end */
    ...
    if ( pMob->game_fun )
    {
	sprintf( buf, "Game fun:    [%s]\n\r",  game_name( pMob->game_fun ) );
	add_buf( buffer, buf );
    }

    if ( pMob->clan )
    {
    	sprintf( buf, "Clan:        [%s]\n\r",  get_clan_table( pMob->clan )->name );
    	add_buf( buffer, buf );
    }
	...
	pRepair = pMob->pRepair;

	sprintf( buf, "Repair data for [%5d]:\n\r", pRepair->keeper );
	add_buf( buffer, buf );
	...
	for ( iTrade = 0; iTrade < MAX_TRADE; iTrade++ )
	{
	    if ( pRepair->repair_type[iTrade] != 0 )
	    {
		if ( iTrade == 0 ) {
		    add_buf( buffer, "  Number Trades Type\n\r" );
		    add_buf( buffer, "  ------ -----------\n\r" );
		...
		sprintf( buf, "  [%4d] %s\n\r", iTrade,
		    flag_string( type_flags, pRepair->repair_type[iTrade] ) );
		add_buf( buffer, buf );
	    }
	}
    }
...
MEDIT( medit_create )
{
    ...
    pMob->vnum			= value;
    pMob->area			= pArea;
    pMob->act			= ACT_IS_NPC | (IS_SET(pArea->area_flags,
    			AREA_PROTOTIPO) ? ACT_PROTOTIPO : 0);
...
MEDIT( medit_level )
{
    ...
    if ( atoi( argument ) < 0 || atoi( argument ) > MAX_LEVEL )
    {
    	send_to_char( "Valores entre 0 y MAX_LEVEL solamente.\n\r", ch );
    	return FALSE;
    }
...
MEDIT( medit_shop )
{
	...
	send_to_char( "         shop type [#x0-4] none\n\r", ch );
...
MEDIT( medit_sex )          /* Moved out of medit() due to naming conflicts -- Hugin */
{
	...
	if ( ( value = sex_lookup( argument ) ) != -1 )
	{
	    pMob->sex = value;
...
MEDIT( medit_size )
{
	...
	if ( ( value = size_lookup(argument) ) != -1 )
	{
	    pMob->size = value;
...
MEDIT( medit_position )
{
	...
	if ( ( value = position_lookup(argument) ) == -1 )
	    break;
    ...
    send_to_char( "Syntax:  position [start/default] [position]\n\r"
		  "Type 'poslist' for a list of positions.\n\r", ch );
    return FALSE;
}
...
MEDIT( medit_clan )
{
	...
	if ( argument[0] == '\0' )
	{
		send_to_char( "Sintaxis: clan [nombre-del-clan]\n\r", ch );
		send_to_char( "          clan ninguno\n\r", ch );
		return FALSE;
	}
	
	clan = clan_lookup(argument);

	if ( clan == 0 && str_cmp(argument,"ninguno") )
	{
		send_to_char( "Ese no es un clan valido.\n\r", ch );
		return FALSE;
	}
...
MEDIT( medit_repair )
{
    ...
    if ( command[0] == '\0' )
    {
	send_to_char( "Syntax:  repair hours [#xopening] [#xclosing]\n\r", ch );
	send_to_char( "         repair type [#x0-4] [item type]\n\r", ch );
	send_to_char( "         repair assign\n\r", ch );
	send_to_char( "         repair remove\n\r", ch );
	return FALSE;
    }
	...
	if ( !pMob->pRepair )
	{
	    send_to_char( "MEdit:  Debes crear un repair primero (repair assign).\n\r", ch );
	    return FALSE;
	}

	pMob->pRepair->open_hour = atoi( arg1 );
	pMob->pRepair->close_hour = atoi( argument );
	...
	|| argument[0] == '\0' )
	{
	    send_to_char( "Syntax:  repair type [#x0-4] [item type]\n\r", ch );
	    return FALSE;
	}
	...
	pMob->pRepair->repair_type[atoi( arg1 )] = value;

	send_to_char( "Repair type set.\n\r", ch);
	return TRUE;
    }

    /* repair assign && repair delete by Phoenix */

    if ( !str_prefix(command, "assign") )
    {
    	if ( pMob->pRepair )
    	{
        	send_to_char("Mob already has a repair assigned to it.\n\r", ch);
        	return FALSE;
	}

	pMob->pRepair		= new_repair();
	if ( !repair_first )
        	repair_first		= pMob->pRepair;
	if ( repair_last )
		repair_last->next	= pMob->pRepair;
	repair_last		= pMob->pRepair;

	pMob->pRepair->keeper	= pMob->vnum;

	send_to_char("New repair assigned to mobile.\n\r", ch);
	return TRUE;
    }
	...
	if ( pRepair == repair_first )
	{
		if ( !pRepair->next )
		{
			...
			repair_first = pRepair->next;
	}
	else
	{
		REPAIR_DATA *ipRepair;

		for ( ipRepair = repair_first; ipRepair; ipRepair = ipRepair->next )
		{
			if ( ipRepair->next == pRepair )
			{
				if ( !pRepair->next )
				{
					repair_last = ipRepair;
					repair_last->next = NULL;
				}
				else
					ipRepair->next = pRepair->next;
			}
		}
	}
	...
	send_to_char("Mobile is no longer a repairkeeper.\n\r", ch);
	return TRUE;
    }
...
OEDIT( oedit_clan )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Sintaxis : clan [nombre-clan]\n\r", ch );
		return FALSE;
	}
...
MEDIT( medit_script )
{
	...
	{
		send_to_char( "Sintaxis : script [vnum-script]\n\r", ch );
		return FALSE;
	}
	...
	{
		send_to_char( "MEdit : Script inexistente.\n\r", ch );
		return FALSE;
	}

	mob->script = script;
	send_to_char( "Script asignado al mob.\n\r", ch );
	return TRUE;
}
...
OEDIT ( oedit_addoprog )
{
	...
	if (!is_number(num) || trigger[0] =='\0' || phrase[0] =='\0' )
	{
		send_to_char("Sintaxis: addoprog [vnum] [trigger] [frase/porcentaje]\n\r", ch);
		return FALSE;
	}

	if ( (value = flag_value (oprog_flags, trigger) ) == NO_FLAG)
	{
		send_to_char("OEdit : Flag invalida. Flags validas son:\n\r",ch);
		show_help( ch, "oprog");
		return FALSE;
	}
	...
	if ( ( code =get_mprog_index (atoi(num) ) ) == NULL)
	{
		send_to_char("OEdit : OBJProgram inexistente.\n\r",ch);
		return FALSE;
	}
...
OEDIT ( oedit_deloprog )
{
	...
	if ( value < 0 )
	{
		send_to_char("OEdit : Solo valores positivos para los oprogs son validos.\n\r",ch);
		return FALSE;
	}

	if ( !(list= pObj->mprogs) )
	{
		send_to_char("OEdit : OProg inexistente.\n\r",ch);
		return FALSE;
	}
...
REDIT ( redit_addrprog )
{
	...
	if (!is_number(num) || trigger[0] =='\0' || phrase[0] =='\0' )
	{
		send_to_char("Sintaxis: addrprog [vnum] [trigger] [frase/porcentaje]\n\r", ch);
		return FALSE;
	}

	if ( (value = flag_value (rprog_flags, trigger) ) == NO_FLAG)
	{
		send_to_char("REdit : Flag invalida. Flags validas son:\n\r",ch);
		show_help( ch, "rprog");
		return FALSE;
	}
	...
	if ( ( code =get_mprog_index (atoi(num) ) ) == NULL)
	{
		send_to_char("REdit : ROOMProgram inexistente.\n\r",ch);
		return FALSE;
	}
	...
	list->trig_phrase	= str_dup(phrase);
	list->code		= code->code;
	SET_BIT(pRoom->mprog_flags,value);
...
REDIT ( redit_delrprog )
{
	...
	if ( value < 0 )
	{
		send_to_char("REdit : Solo valores positivos para los rprogs son validos.\n\r",ch);
		return FALSE;
	}

	if ( !(list= pRoom->mprogs) )
	{
		send_to_char("REdit : RProg inexistente.\n\r",ch);
		return FALSE;
	}

	if ( value == 0)
	{
		REMOVE_BIT(pRoom->mprog_flags, pRoom->mprogs->trig_type);
		list = pRoom->mprogs;
		pRoom->mprogs = list->next;
		free_mprog( list );
	}
	else
	{
		...
		if ( list_next )
		{
			REMOVE_BIT(pRoom->mprog_flags, list_next->trig_type);
			list->next = list_next->next;
			free_mprog(list_next);
		}
		else
...
MEDIT( medit_norecalc )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Sintaxis : norecalc [si/no]\n\r", ch );
		return FALSE;
	}

	if ( !str_cmp(argument, "si") )
	{
		send_to_char( "Ok.\n\r", ch );
		mob->norecalc = TRUE;
		return TRUE;
	...
	else if ( !str_cmp(argument, "no") )
	{
		...
		send_to_char( "MEdit : Argumento invalido.\n\r", ch );
...
ED_FUN_DEC(ed_line_string)
{
	...
	if (IS_NULLSTR(argument))
	{
		sprintf( buf, "Sintaxis : %s <cadena>\n\r", olc_comm_table[num].nombre );
		send_to_char( buf, ch );
		return FALSE;
	}

	free_string(*string);
	*string = str_dup( argument );
...
bool numedit( int num, CHAR_DATA *ch, char *argument, void * arg, sh_int tipo, long min, long max )
{
	int temp;
	int * valor = (int *) arg;
...
{
	return numedit( num, ch, argument, arg, CORTO, -1000, 1000 );
}

ED_FUN_DEC(ed_numero_pos)
{
	return numedit( num, ch, argument, arg, NORMAL, 0, -1 );
}

ED_FUN_DEC(ed_numero_s_pos)
{
	return numedit( num, ch, argument, arg, CORTO, 0, -1 );
}

ED_FUN_DEC(ed_numero_l_pos)
{
	return numedit( num, ch, argument, arg, LARGO, 0, -1 );
}

ED_FUN_DEC(ed_numero_niv)
{
	return numedit( num, ch, argument, arg, CORTO, 0, MAX_LEVEL );
}

ED_FUN_DEC(ed_desc_mob)
{
    MOB_INDEX_DATA * pMob;
	...
	if ( ( value = flag_value( (struct flag_type *) par, argument ) ) != NO_FLAG )
	{
	    *(long *) arg ^= value;
	    ...
	    printf_to_char( ch, "%c%s flag toggled.\n\r",
	    	toupper(olc_comm_table[num].nombre[0]),
	    	&olc_comm_table[num].nombre[1] );
	    return TRUE;
	}
    }
...
ED_FUN_DEC( ed_shop )
{
    ...
    ed_shop( num, ch, "", arg, par );

    return FALSE;
}

ED_FUN_DEC( ed_repair )
{
    MOB_INDEX_DATA *pMob = (MOB_INDEX_DATA *) arg;
    char command[MAX_INPUT_LENGTH];
    char arg1[MAX_INPUT_LENGTH];
...
ED_FUN_DEC( ed_gamespec )
{
    ...
    GAME_FUN ** game = (GAME_FUN **) arg;

    if ( argument[0] == '\0' )
    {
	printf_to_char( ch, "Sintaxis : %s [%s]\n\r",
		olc_comm_table[num].nombre,
		olc_comm_table[num].nombre );
	return FALSE;
    }
    ...
    else
    {
    	if ( game_lookup(argument) )
    	{
		* game = game_lookup(argument);
    		send_to_char( "Game seteado.\n\r", ch );
    		return TRUE;
    	}
    	else
    	{
    		send_to_char( "ERROR : Game inexistente.\n\r", ch );
    		return FALSE;
    	}
    }
...
bool templookup( int num, CHAR_DATA *ch, char *argument, void *arg, const void *par, int temp )
{
    int value;
...
{
	return templookup( num, ch, argument, arg, par, 0 );
}

ED_FUN_DEC( ed_shintlookup )
{
	return templookup( num, ch, argument, arg, par, -1 );
}
...
ED_FUN_DEC(ed_dice)
{
    ...
    printf_to_char( ch, "%sdice seteado.\n\r",
    	olc_comm_table[num].nombre );
...
ED_FUN_DEC( ed_addprog )
{
  ...
  if (!is_number(numb) || trigger[0] =='\0' || phrase[0] =='\0' )
  {
        send_to_char("Syntax:   addmprog [vnum] [trigger] [phrase]\n\r",ch);
        return FALSE;
  }
  ...
  if ( ( code =get_mprog_index (atoi(numb) ) ) == NULL)
  {
        send_to_char("No such MOBProgram.\n\r",ch);
        return FALSE;
  }
...
ED_FUN_DEC(ed_delprog)
{
	...
	case ED_OBJECT:
	EDIT_OBJ(ch, pObj);
	REMOVE_BIT(pObj->mprog_flags,t2rem);
	break;

	case ED_ROOM:
	EDIT_ROOM(ch, pRoom);
	REMOVE_BIT(pRoom->mprog_flags,t2rem);
	break;
  }
...
ED_FUN_DEC( ed_script )
{
	SCRIPT_DATA ** pScript = (SCRIPT_DATA **) arg;
	int vnum;
	SCRIPT_DATA *script;
...
ED_FUN_DEC( ed_version )
{
	return show_version( ch, "" );
}
...
ED_FUN_DEC( ed_trap )
{
    char blarg[MIL];
    int value;
    TRAP_DATA **pTrap = (TRAP_DATA **) arg;

    if ( IS_NULLSTR(argument) )
    {
    	send_to_char( "Sintaxis : trap set\n\r", ch );
    	send_to_char( "           trap remove\n\r", ch );
    	send_to_char( "           trap charges [cargas]\n\r", ch );
    	send_to_char( "           trap damage [tipo dam]\n\r", ch );
    	send_to_char( "           trap effect [tipo eff]\n\r", ch );
    	return FALSE;
    }

    argument = one_argument( argument, blarg );

    if ( !str_cmp( blarg, "charges" ) )
    {
	if ( !*pTrap )
		*pTrap = new_trap();

	(*pTrap)->trap_charge = atoi( argument );
	...
	*pTrap	= NULL;
	...
	if ( ( value = flag_value( trap_flags, argument ) ) != NO_FLAG )
	{
	    ...
	    TOGGLE_BIT((*pTrap)->trap_eff, value);
