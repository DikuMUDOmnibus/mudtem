...
void ban_site(CHAR_DATA *ch, char *argument, bool fPerm)
{
	...
	if (ban_list == NULL)
	{
	    send_to_char("No hay sitios baneados en este momento.\n\r",ch);
	    return;
  	}
	buffer = new_buf();
    ...
    {
	send_to_char("Tipos de ban aceptables son all, newbies, y permit.\n\r",
	    ch); 
	return;
    }
    ...
    if (strlen(name) == 0)
    {
	send_to_char("Debes banear ALGO.\n\r",ch);
	return;
    }
    ...
    for ( pban = ban_list; pban != NULL; prev = pban, pban = pban->next )
    {
	    ...
	    {
            	send_to_char( "Ese ban fue puesto por un poder mayor.\n\r", ch );
            	return;
	    }
	    else
	    {
...
void do_allow( CHAR_DATA *ch, char *argument )                        
{
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char( "Sacar cual sitio de la lista de bans?\n\r", ch );
        return;
    }
    ...
    for ( curr = ban_list; curr != NULL; prev = curr, curr = curr->next )
    {
	    ...
	    {
		send_to_char(
		   "No eres lo suficientemente poderoso para levantar ese ban.\n\r",ch);
		return;
	    }
            if ( prev == NULL )
            ...
            free_ban(curr);
	    sprintf(buf,"Ban para %s levantado.\n\r",arg);
            send_to_char( buf, ch );
	    save_bans();
            return;
        }
    ...
    send_to_char( "Ese sitio no esta baneado.\n\r", ch );
    return;
}
