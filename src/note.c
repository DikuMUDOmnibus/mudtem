...
void do_unread(CHAR_DATA *ch)
{
    ...
    {
	found = TRUE;
	sprintf(buf,"Hay %d nueva%s Noticia%s (#BNEWS#b) esperando.\n\r",
    ...
    {
	found = TRUE;
	sprintf(buf,"Hay %d nuevo%s Cambio%s (#BCHANGE#b) para leer.\n\r",
	    count, count > 1 ? "s" : "", count > 1 ? "s" : "" );
        send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"Tienes %d nueva%s nota%s (#BNOTE#b) esperando.\n\r",
	    count, count > 1 ? "s" : "", count > 1 ? "s" : "" );
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"Tienes %d nueva%s idea%s (#BIDEA#b) para evaluar.\n\r",
	    count, count > 1 ? "s" : "", count > 1 ? "s" : "" );
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"%d castigo%s (#BPENALTY#b) ha%s sido anadido%s.\n\r",
	    count, count > 1 ? "s" : "", count > 1 ? "s" : "",
	    count > 1 ? "s" : "" );
	send_to_char(buf,ch);
    }
    ...
    if (!found)
	send_to_char("No hay mensajes nuevos para leer.\n\r",ch);
}
...
bool is_note_to( CHAR_DATA *ch, NOTE_DATA *pnote )
{
    ...
    if (ch->clan && is_exact_name(get_clan_table(ch->clan)->name,pnote->to_list))
	return TRUE;
...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
	...
	sprintf(buf,"No hay esa cantidad de %s.\n\r",list_name);
	send_to_char(buf,ch);
        return;
    }
	...
	if (!vnum)
	{
	    ...
	    {
		case NOTE_NOTE:	
		    send_to_char("No hay notas para ti.\n\r",ch);
		    break;
		case NOTE_IDEA:
		    send_to_char("No hay ideas para ti.\n\r",ch);
		    break;
		case NOTE_PENALTY:
		    send_to_char("No hay castigos para ti.\n\r",ch);
		    break;
		case NOTE_NEWS:
		    send_to_char("No hay noticias para ti.\n\r",ch);
		    break;
		case NOTE_CHANGES:
		    send_to_char("No hay cambios para ti.\n\r",ch);
		    break;
	    }
	}
	return;
    ...
    if ( !str_prefix( arg, "remove" ) )
    {
        ...
        {
            send_to_char( "Remover que numero?\n\r", ch );
            return;
        }
    ...
    if ( !str_prefix( arg, "delete" ) && get_trust(ch) >= MAX_LEVEL - 1)
    {
        if ( !is_number( argument ) )
        {
            send_to_char( "Eliminar que numero?\n\r", ch );
            return;
        }
    ...
    {
	sprintf(buf,"Tu nivel no es lo suficientemente alto como para escribir %s.",list_name);
	send_to_char(buf,ch);
	return;
    }
    ...
    if ( !str_cmp( arg, "edit" ) )
    {
    	...
    	{
    		send_to_char( "Ya estas escribiendo otra nota.\n\r", ch );
    		return;
    	}
	...
	{
	    send_to_char(
		"Ya estas escribiendo otra nota.\n\r",ch);
	    return;
	}

	if (strlen(ch->pnote->text)+strlen(argument) >= 4096)
	{
	    send_to_char( "Nota demasiado larga.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char("No hay lineas para borrar.\n\r",ch);
	    return;
	}
	...
	note_attach( ch,type );
        ...
        {
            send_to_char(
                "Ya estas escribiendo otra note.\n\r",ch);
            return;
        }
	...
	if ( ch->pnote == NULL )
	{
	    send_to_char( "No estas escribiendo notas.\n\r", ch );
	    return;
	}

	if (ch->pnote->type != type)
	{
	    send_to_char("No estas escribiendo ese tipo de nota.\n\r",ch);
	    return;
	}
	...
	if ( ch->pnote == NULL )
	{
        ...
        if (ch->pnote->type != type)
        {
            send_to_char("No estas escribiendo este tipo de nota.\n\r",ch);
            return;
        }
	...
	{
	    send_to_char(
		"Debes poner un destino (name, all, o immortal).\n\r",
		ch);
	    return;
	}
	...
	{
	    send_to_char("Debes poner un tema.\n\r",ch);
	    return;
	}
