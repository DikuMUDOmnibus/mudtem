...
const	struct	olc_comm_type	skill_olc_comm_table	[]	=
{
 ...
 { "nombre",	NULL,				ed_olded,		skedit_nombre	},
 ...
 { "room",	(void *) &xSkill.msg_room,	ed_line_string,		NULL		},
 { "new",	NULL,				ed_olded,		skedit_new	},
 { "callback",	NULL,				ed_olded,		skedit_callback	},
 { "flags",	(void *) &xSkill.flags,		ed_flag_toggle,		skill_flags	},
#if defined(MUD_SLANG)
 { "slang",	(void *) &xSkill.slangscript,	ed_desc,		NULL,		},
#endif
...
DECLARE_SPELL_CB(vampiric_bite_cb)
{
...
const struct spcb_type spcb_table [] =
{
	{	"vampiric_bite_cb",	vampiric_bite_cb	},
	{	NULL,			NULL			}
};

char * spcb_name_lookup( SPELL_CB * spcb )
{
	int i;

	for ( i = 0; spcb_table[i].nombre; i++ )
		if ( spcb_table[i].funcion == spcb )
			return spcb_table[i].nombre;

	return "";
}

SPELL_CB * spcb_lookup( char *argument )
{
	int i;
	...
	for ( i = 0; spcb_table[i].nombre; ++i )
		if ( !str_cmp(spcb_table[i].nombre, argument) )
			return spcb_table[i].funcion;
...
SKEDIT( skedit_nombre )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Sintaxis : nombre [nombre]\n\r", ch );
		return FALSE;
	}
	...
	free_string(pSkill->nombre);
	pSkill->nombre = str_dup(argument);
...
SKEDIT( skedit_show )
{
	...
	sprintf( buf,	"Name     : [%s]\n\r"
			"Nombre   : [%s]\n\r",
		pSkill->name, pSkill->nombre );
	send_to_char( buf, ch );

	sprintf( buf,   "Flags    : [%s]\n\r",
		flag_string( skill_flags, pSkill->flags ) );
	send_to_char( buf, ch );
	...
	sprintf( buf, "Msg Room : [%s]\n\r", pSkill->msg_room );
	send_to_char( buf, ch );

	sprintf( buf, "Callback : [%s]\n\r", spcb_name_lookup(pSkill->spell_callback) );
	send_to_char( buf, ch );

#if defined(MUD_SLANG)
	if (!IS_NULLSTR(pSkill->slangscript))
		send_to_char( "Spell tiene #BS#b-#BL#bang#BS#bcript.\n\r", ch );
#endif
...
SKEDIT( skedit_new )
{
	...
	skill_table[MAX_SKILL-1].msg_room		= str_dup( "" );
...
SKEDIT( skedit_callback )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Sintaxis : callback [nombre-cb]\n\r", ch );
		send_to_char( "           callback spell_null\n\r", ch );
		return FALSE;
	}
	
	if ( ( spcb = spcb_lookup(argument) ) == NULL )
	{
		send_to_char( "SKEdit : Callback inexistente.\n\r", ch );
		return FALSE;
	}
	
	pSkill->spell_callback = spcb;
