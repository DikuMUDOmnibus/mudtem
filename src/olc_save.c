...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    temp = DIF( pMobIndex->affected2_by, race_table[race].aff2 );
    fprintf( fp, "%s ",		  fwrite_flag( temp,	buf ) );
    ...
    if (pMobIndex->clan > 0)
	 fprintf ( fp, "L '%s'\n" , get_clan_table(pMobIndex->clan)->name );

    fprintf( fp, "C '%s'\n",	class_table[pMobIndex->clase].name );
    ...
    if (pMobIndex->script && pMobIndex->script->comandos
    && !IS_NULLSTR(pMobIndex->script->comandos[0]))
    {
	fprintf( fp, "S %d\n", pMobIndex->script->vnum );
	save_script( pMobIndex->vnum );
    }

    if (pMobIndex->norecalc)
    	fprintf( fp, "R\n" );
...
void save_object( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
    ...
    fprintf( fp, "%s~\n",	pObjIndex->material != -1 ?
    				flag_string(mat_table, pObjIndex->material) :
    				pObjIndex->material_string );
 ...
 */
    ...
    {
    	fprintf( fp, "T %d %d %d\n",    pObjIndex->trap->trap_eff, pObjIndex->trap->trap_dam,
					pObjIndex->trap->trap_charge );
    }
	...
	LIMIT_DATA * limit;
    	...
    	fprintf( fp, "R 0 %d\n", pObjIndex->max_count );
    	for ( limit = pObjIndex->limit; limit; limit = limit->next )
    		fprintf( fp, "L %ld %d\n", limit->carrier_id, limit->id );
    }

    if (pObjIndex->clan)
	fprintf( fp, "N '%s'\n", get_clan_table(pObjIndex->clan)->name );

    for ( pMprog = pObjIndex->mprogs; pMprog; pMprog = pMprog->next )
    {
        fprintf(fp, "M %s %d %s~\n",
        	oprog_type_to_name(pMprog->trig_type),
        	pMprog->vnum,
                pMprog->trig_phrase);
    }
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
		...
		if (pRoomIndex->affected)
		{
			...
			for ( pAf = pRoomIndex->affected; pAf; pAf = pAf->next )
				fprintf( fp, "A '%s' %3d %3d %3d %3d %3d %10d\n",
		...
		for ( pMprog = pRoomIndex->mprogs; pMprog; pMprog = pMprog->next )
		{
			fprintf(fp, "R %s %d %s~\n",
				rprog_type_to_name(pMprog->trig_type),
				pMprog->vnum,
				pMprog->trig_phrase);
		}
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for( pMobIndex = mob_index_hash[iHash]; pMobIndex; pMobIndex = pMobIndex->next )
        {
            if ( pMobIndex && pMobIndex->area == pArea && pMobIndex->pShop )
            {
                ...
                fprintf( fp, "%d %d ", pShopIndex->profit_buy_actual, pShopIndex->profit_sell_actual );
...
void save_repairs( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for( pMobIndex = mob_index_hash[iHash]; pMobIndex; pMobIndex = pMobIndex->next )
        {
            ...
            {
                pRepairIndex = pMobIndex->pRepair;

                fprintf( fp, "%d ", pRepairIndex->keeper );
                for ( iTrade = 0; iTrade < MAX_TRADE; iTrade++ )
                {
                    if ( pRepairIndex->repair_type[iTrade] != 0 )
                    {
                       fprintf( fp, "%d ", pRepairIndex->repair_type[iTrade] );
                    }
                    else
                       fprintf( fp, "0 ");
                }
                fprintf( fp, "%d %d\n", pRepairIndex->open_hour, pRepairIndex->close_hour );
            }
        }
    }
...
void save_area( AREA_DATA *pArea )
{
	...
	fprintf( fp, "Prototipo %d\n",        IS_SET(pArea->area_flags, AREA_PROTOTIPO) ? 1 : 0 );
	fprintf( fp, "Low %d\n",		  pArea->low_range );
	fprintf( fp, "High %d\n",		  pArea->high_range );
	if (!IS_NULLSTR(pArea->clan))
		fprintf( fp, "Clan %s~\n",	pArea->clan );
