...
/*    pArea->recall           =   ROOM_VNUM_TEMPLE;      ROM OLC */
    pArea->area_flags	= AREA_ADDED | AREA_PROTOTIPO;
    pArea->security	= 1;
...
void free_exit( ROOM_INDEX_DATA *room, EXIT_DATA *pExit )
{
    free_string( pExit->keyword );
    free_string( pExit->description );

    if ( room->exits == pExit )
    	room->exits	= pExit->next;
    else
    {
	EXIT_DATA *prev;

    	for ( prev = room->exits; prev; prev = prev->next )
    		if ( prev->next == pExit )
    			break;

	if ( prev )
		prev->next = pExit->next;
	else
		bugf( "free_exit : prev no encontrado" );
    }
...
void free_room_index( ROOM_INDEX_DATA *pRoom )
{
    ...
    RESET_DATA *pReset;
    EXIT_DATA *pExit, *pNext;

    free_string( pRoom->name );
    free_string( pRoom->description );
    free_string( pRoom->owner );

    for ( pExit = pRoom->exits; pExit; pExit = pNext )
    {
    	    pNext = pExit->next;
...
REPAIR_DATA *new_repair( void )
{
    ...
    for ( buy=0; buy<MAX_TRADE; buy++ )
        pRepair->repair_type[buy]    =   0;

    pRepair->open_hour    =   0;
    pRepair->close_hour   =   23;
...
void free_mpcode(MPROG_CODE *pMcode)
{
    ...
    free_string(pMcode->descripcion);
