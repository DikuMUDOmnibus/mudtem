...
SCRIPT_DATA *script_pedir( int );
void show_races(CHAR_DATA *);
COMMAND(do_clear)

bool puede_crear_cuarto(CHAR_DATA *, int);
ROOM_INDEX_DATA * crear_cuarto(int);
...
const struct olc_help_type help_table[] =
{
    ...
    {	"rprog",	rprog_flags,	"Tipos de Room Progs."		},
    {	"oprog",	oprog_flags,	"Tipos de Obj Progs."		},
...
bool show_help( CHAR_DATA *ch, char *argument )
{
          ...
          && !str_prefix( arg, help_table[cnt].command ) )
	{
	    ...
	    if ( !str_prefix(arg, "race") && help_table[cnt].structure == NULL )
	    {
	    	show_races( ch );
	    	return FALSE;
	    }
	    else
	    if ( !str_prefix(arg, "spell") && help_table[cnt].structure == NULL )
	    {
...
bool change_exit( CHAR_DATA *ch, char *argument, int door )
{
	...
	printf_to_char( ch, "Salida %s del cuarto %d borrada.\n\r",
		dir_nombre[pExit->direccion], pRoom->vnum );
	free_exit( pRoom, pExit );

	if (rDeleted)
		printf_to_char( ch, "Salida %s del cuarto %d tambien fue borrada.\n\r",
			dir_nombre[rev_dir[pExit->direccion]], pToRoom->vnum );
	...
	if ( !pRoomIndex )
	{
	    send_to_char( "REdit: No puedes linkear a un cuarto que no existe.\n\r", ch );
	    return FALSE;
	}

	if ( !IS_BUILDER(ch, pRoomIndex->area) )
	{
	    send_to_char( "REdit: No puedes linkear a esa area.\n\r", ch );
	    return FALSE;
	}
	...
	if ( pExit )
	{
	    send_to_char( "REdit: Salida opuesta en el cuarto destino ya existe.\n\r", ch );
	    return FALSE;
	}
	...
	send_to_char( "Salida en ambos sentidos creada.\n\r", ch );
	return TRUE;
    }
	...
	if ( arg[0] == '\0' || !is_number( arg ) || (value = atoi(arg)) < 1 )
	{
	    send_to_char( "Syntax: [direction] dig <vnum>\n\r", ch );
	    return FALSE;
	}

	if (puede_crear_cuarto(ch, value))
	{
		crear_cuarto(value);
	...
	if ( (pExit = exit_lookup(target, rev_dir[door])) != NULL
	&&    pExit->u1.to_room != pRoom )
...
bool puede_crear_cuarto(CHAR_DATA *ch, int value)
{
    AREA_DATA * pArea;
    ...
    if ( !pArea )
    {
	printf_to_char( ch, "REdit : Vnum %d no esta asignado a ningun area.\n\r", value );
	return FALSE;
    }

    if ( !IS_BUILDER( ch, pArea ) )
    {
	send_to_char( "REdit : Vnum en un area que no puedes editar.\n\r", ch );
	return FALSE;
    }

    if ( get_room_index( value ) )
    {
	send_to_char( "REdit : Un cuarto con ese vnum ya existe.\n\r", ch );
	return FALSE;
    }
...
ROOM_INDEX_DATA * crear_cuarto(int value)
{
    ROOM_INDEX_DATA *pRoom;
    AREA_DATA * pArea;
    int iHash;
...
REDIT( redit_mreset )
{
    ...
    act( "$n has created $N!", ch, NULL, chToEnt(newmob), TO_ROOM );
    return TRUE;
}
...
REDIT( redit_oreset )
{
	...
	olevel  = URANGE( 0, getNivelPr(to_mob) - 2, LEVEL_HERO );
        newobj = create_object( pObjIndex, number_fuzzy( olevel ) );
    ...
    act( "$n has created $p!", ch, objToEnt(newobj), NULL, TO_ROOM );
    return TRUE;
}
 ...
 */
char * show_obj_values( OBJ_INDEX_DATA *obj )
{
...
/* OLC		    do_help( ch, "liquids" );    */
    ...
    send_to_char(show_obj_values( pObj ),ch);
...
OEDIT( oedit_show )
{
	...
	flag_string( mat_table, pObj->material ) );
    send_to_char( buf, ch );
...
MEDIT(medit_copy)
{
	...
	pMob->norecalc		= mob2->norecalc;
	pMob->act		= mob2->act;
	pMob->affected_by	= mob2->affected_by;
	pMob->affected2_by	= mob2->affected2_by;
...
void reset_pflags( long * pint, MPROG_LIST * mpl )
{
	long bitv = 0;

	for ( ; mpl; mpl = mpl->next )
		SET_BIT(bitv, mpl->trig_type);
...
ED_FUN_DEC( ed_addprog )
{
  ...
  switch(ch->desc->editor)
  {
  	...
  	case ED_OBJECT: flagtable = oprog_flags;	break;
  	case ED_ROOM:	flagtable = rprog_flags;	break;
  ...
  *mprogs		= list;
  ...
  switch(ch->desc->editor)
  {
  	case ED_MOBILE:
  	EDIT_MOB(ch, pMob);
	reset_pflags(&(pMob->mprog_flags),*mprogs);
	break;

	case ED_OBJECT:
	EDIT_OBJ(ch, pObj);
	reset_pflags(&(pObj->mprog_flags),*mprogs);
	break;

	case ED_ROOM:
	EDIT_ROOM(ch, pRoom);
	reset_pflags(&(pRoom->mprog_flags),*mprogs);
	break;
  }
...
ED_FUN_DEC( ed_script )
{
	...
	if ( (script = script_pedir( vnum )) == NULL )
	{
...
ED_FUN_DEC( ed_trap )
{
	...
	*pTrap	= NULL;
    ...
    return ed_trap( n_fun, ch, "", arg, par );
}

void show_races( CHAR_DATA * ch )
{
	int race;
...
REDIT( redit_limpiar )
{
	...
	for ( pExit = pRoom->exits; pExit; pExit = pExit_next )
	{
