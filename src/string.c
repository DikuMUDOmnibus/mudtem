...
{
    send_to_char( "-=============- Entrando en el modo de EDICION -==============-\n\r", ch );
    send_to_char( "      Escribe #B.h#b en una linea en blanco para ver la ayuda\n\r", ch );
    send_to_char( "Para salir del modo de edicion escribe #B@#b en una linea en blanco.\n\r", ch );
    send_to_char( "-==============================================================-\n\r", ch );
...
/*            **ch->desc->pString = '\0'; */
	...
	if (ch->desc->showstr_head)
        {
        	write_to_buffer( ch->desc, "[#B!!!#b] Los siguientes mensajes aparecieron mientras estabas editando:\n\r", 0 );
        	show_string(ch->desc,"");
        }
	...
	if ( ch->desc->editor == ED_PROG && ch->desc->pEdit )
	{
		...
		for ( ; hash < MAX_KEY_HASH; hash++ )
		{
			...
			for ( obj = obj_index_hash[hash]; obj; obj = obj->next )
				for ( mp = obj->mprogs; mp; mp = mp->next )
					if ( mp->vnum == mpc->vnum )
					{
						mp->code = mpc->code;
						printf_to_char( ch, "Arreglando obj %d.\n\r", obj->vnum );
					}

			for ( room = room_index_hash[hash]; room; room = room->next )
				for ( mp = room->mprogs; mp; mp = mp->next )
					if ( mp->vnum == mpc->vnum )
					{
						mp->code = mpc->code;
						printf_to_char( ch, "Arreglando room %d.\n\r", room->vnum );
					}
		}
	}
