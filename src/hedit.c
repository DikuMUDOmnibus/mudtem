...
void hedit( CHAR_DATA *ch, char *argument)
{
    ...
    if ( (hArea = get_help_area(pHelp)) == NULL )
    {
    	send_to_char( "HEdit : Area nula!\n\r", ch );
    	edit_done(ch);
    	return;
    }
	...
	if (!str_prefix(command, hedit_table[cmd].name) )
	{
		if ((*hedit_table[cmd].olc_fun) (ch, argument))
		{
			...
			if ( hArea->area )
				SET_BIT(hArea->area->area_flags, AREA_CHANGED);
		}
		return;
	}
    }
...
HEDIT(hedit_delete)
{
	...
	flog( "Borrando help %s, jugador %s.", pHelp->keyword, ch->name );
