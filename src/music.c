...
void song_update(void)
{
	...
	else
	{
	    ...
	    {
	    	sprintf(buf,"#BCancion#b: #B%s#b, #B%s#b",
		...
		victim = d->original ? d->original : d->character;
        	...
        	if ( d->connected == CON_PLAYING &&
            	     ...
            	     act_new("$t",d->character,strToEnt(buf,d->character->in_room),NULL,TO_CHAR,POS_SLEEPING);
            }
	}
    }
	...
	if (obj->value[0] < 0)
	{
	    sprintf(buf,"$p empieza a tocar #B%s#b, #B%s#b.",
		song_table[obj->value[1]].group,song_table[obj->value[1]].name);
	    if (room->people != NULL)
		act(buf,room->people,objToEnt(obj),NULL,TO_ALL);
	    obj->value[0] = 0;
	    continue;
	}
	else
...
void do_play(CHAR_DATA *ch, char *argument)
{
    ...
    if (argument[0] == '\0')
    {
	send_to_char("Tocar que?\n\r",ch);
	return;
    }

    if (juke == NULL)
    {
	send_to_char("No ves nada para tocar.\n\r",ch);
	return;
    }
	...
	sprintf(buf,"%s tiene disponibles las siguientes canciones:\n\r",
	    juke->short_descr);
    ...
    {
        send_to_char("El wurlitzer esta lleno en este momento.\n\r",ch);
        return;
    }
	...
	{
	    send_to_char("Esa cancion no esta disponible.\n\r",ch);
	    return;
	}
    ...
    {
    	deduct_cost( ch, cost );
    	act( "$n inserta unas monedas en $p.", ch, objToEnt(juke), NULL, TO_ROOM );
    }

    send_to_char("Ok. Espera unos momentos.\n\r",ch);
...
void do_silencio( CHAR_DATA *ch, char *argument )
{
	int i;
