	...
	switch (cl->cl_class_type)
	  {
	   ...
	   default:
	     ...
	     else if (cl->cl_data_type == SLANG_ENT_TYPE)
	      (void) (*cl->cl_push) (type, (VOID_STAR) ret);
...
lang_do_loops (unsigned char stype, SLBlock_Type *block, unsigned int num_blocks)
{
   ...
   switch (stype)
	 ...
	 */
	if ((-1 == _SLroll_stack (-(Next_Function_Num_Args + 1)))
...
int _SLpush_slang_obj (SLang_Object_Type *obj)
{
   ...
   if (cl->cl_data_type == SLANG_ENT_TYPE)
   	return (*cl->cl_push) (subtype, (VOID_STAR) obj->v.ptr_val);
   else
...
_INLINE_
static void do_bc_call_direct_frame (int (*f)(void))
{
...
static SLang_Name_Type **Globals_Hash_Table;
...
static _SLNameSpace_List_Type *This_Static_NameSpace;
...
static int push_compile_context (void);
...
typedef struct
{
   ...
   _SLNameSpace_List_Type *static_namespace;
}
Block_Context_Type;
...
static SLang_Name_Type *locate_namespace_encoded_name (char *name)
{
   char *ns, *ns1;
   _SLNameSpace_List_Type *table;
   SLang_Name_Type *nt;
   ...
   if (NULL == (table = _SLns_find_namespace (ns)))
     {
	SLang_verror (SL_SYNTAX_ERROR, "Unable to find namespace called %s",
		      ns);
	SLang_free_slstring (ns);
	return NULL;
     }
...
static SLang_Name_Type *locate_hashed_name (char *name, unsigned long hash)
{
   ...
   if ((This_Static_NameSpace != NULL)
       && (NULL != (t = locate_name_in_table (name, hash, This_Static_NameSpace->table, SLSTATIC_HASH_TABLE_SIZE))))
     return t;

   t = locate_name_in_table (name, hash, Globals_Hash_Table, SLGLOBALS_HASH_TABLE_SIZE);
   if (NULL != t)
     return t;

   return locate_namespace_encoded_name (name);
}
...
add_slang_function (char *name, unsigned char type, unsigned long hash,
		    ...
		    _SLBlock_Header_Type *h, SLang_Name_Type **table,
...
int SLang_execute_function (char *name)
{
   ...
   if (NULL == (entry = _SLlocate_global_name (name)))
     return 0;
...
SLang_Name_Type *SLang_get_function (char *name)
{
   ...
   if (NULL == (entry = locate_namespace_encoded_name (name)))
     return NULL;
...
{
   (void) lang_define_function (name, SLANG_FUNCTION, hash,
				This_Static_NameSpace->table, SLSTATIC_HASH_TABLE_SIZE);
}

static void define_private_function (char *name, unsigned long hash)
{
   (void) lang_define_function (name, SLANG_PFUNCTION, hash,
				This_Static_NameSpace->table, SLSTATIC_HASH_TABLE_SIZE);
}
...
int SLang_is_defined(char *name)
{
   ...
   t = locate_namespace_encoded_name (name);
   if (t == NULL)
     return 0;
				...
				SLang_Name_Type **table, unsigned int table_size)
{
    ...
    */
   /* hash = _SLcompute_string_hash (name); */
   g = locate_name_in_table (name, hash, table, table_size);
...
void SLang_restart (int localv)
{
   ...
   if (This_Compile_Block_Type == COMPILE_BLOCK_TYPE_FUNCTION)
	...
	lang_define_function (NULL, SLANG_FUNCTION, 0, Globals_Hash_Table, SLGLOBALS_HASH_TABLE_SIZE);
	if (lang_free_branch (This_Compile_Block))
	  SLfree((char *)This_Compile_Block);
     }
   Lang_Defining_Function = 0;
			    ...
			    char *name, unsigned long hash)
{
   ...
   if (v == NULL)
	 ...
	 * scope.
	 */
	if ((-1 == add_global_variable (name, SLANG_GVARIABLE, hash,
					This_Static_NameSpace->table,
					SLSTATIC_HASH_TABLE_SIZE))
...
static void compile_public_variable_mode (_SLang_Token_Type *t)
{
   if (t->type == IDENT_TOKEN)
	 ...
	 */
	if ((This_Static_NameSpace != NULL)
	    && (NULL != locate_name_in_table (t->v.s_val, t->hash, This_Static_NameSpace->table, SLSTATIC_HASH_TABLE_SIZE)))
	  {
	...
	add_global_variable (t->v.s_val, SLANG_GVARIABLE, t->hash, Globals_Hash_Table, SLGLOBALS_HASH_TABLE_SIZE);
     }
...
{
   if (t->type == IDENT_TOKEN)
     add_global_variable (t->v.s_val, SLANG_GVARIABLE, t->hash, This_Static_NameSpace->table, SLSTATIC_HASH_TABLE_SIZE);
   else if (t->type == CBRACKET_TOKEN)
     Compile_Mode_Function = compile_basic_token_mode;
   else
     SLang_verror (SL_SYNTAX_ERROR, "Misplaced token in variable list");
...
{
   if (t->type == IDENT_TOKEN)
     add_global_variable (t->v.s_val, SLANG_PVARIABLE, t->hash, This_Static_NameSpace->table, SLSTATIC_HASH_TABLE_SIZE);
   else if (t->type == CBRACKET_TOKEN)
     Compile_Mode_Function = compile_basic_token_mode;
   else
     SLang_verror (SL_SYNTAX_ERROR, "Misplaced token in variable list");
...
static void compile_function_mode (_SLang_Token_Type *t)
{
   ...
   if (t->type != IDENT_TOKEN)
     ...
     lang_define_function (t->v.s_val, SLANG_FUNCTION, t->hash, Globals_Hash_Table, SLGLOBALS_HASH_TABLE_SIZE);
...
static int init_interpreter (void)
{
   _SLNameSpace_List_Type *ns;

   if (Globals_Hash_Table != NULL)
     return 0;

   if (NULL == (ns = _SLns_allocate_namespace ("***GLOBAL***", SLGLOBALS_HASH_TABLE_SIZE)))
     return -1;
   Globals_Hash_Table = ns->table;
...
{
   return add_generic_table ((SLang_Name_Type *) tbl, pp, sizeof (SLang_IConstant_Type));
}

#if SLANG_HAS_FLOAT
int SLadd_dconstant_table (SLang_DConstant_Type *tbl, char *pp)
{
   return add_generic_table ((SLang_Name_Type *) tbl, pp, sizeof (SLang_DConstant_Type));
}
#endif
