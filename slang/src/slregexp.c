...
static unsigned char *regexp_looking_at(register unsigned char *str, unsigned char *estr, unsigned char *buf, register int cs)
{
   ...
   while (p != 0)
	...
	switch((unsigned char) p)
	  {
	   ...
	   case OPAREN:
	     Open_Paren_Number++;
	     This_Reg->beg_matches[Open_Paren_Number] = (int) (str - This_Str);
	     break;
	   case CPAREN:
	     n = Open_Paren_Number;
	     while (n > 0)
		  ...
		  Closed_Paren_Matches[n] = 1;
		  This_Reg->end_matches[n] = (unsigned int) (str - (This_Str + This_Reg->beg_matches[n]));
		  break;
	       }
	     break;
#ifdef NOT_LITERAL
	...
	SLMEMCPY(save_closed_matches, Closed_Paren_Matches, sizeof(save_closed_matches));
	save_num_open = Open_Paren_Number;
	while (str >= save_str)
	  {
	     tmpstr = regexp_looking_at(str, estr, buf, cs);
	     if (tmpstr != NULL) return(tmpstr);
	     SLMEMCPY(Closed_Paren_Matches, save_closed_matches, sizeof(Closed_Paren_Matches));
	     Open_Paren_Number = save_num_open;
	     str--;
	  }
	return NULL;
     }
...
static void
fixup_beg_end_matches (SLRegexp_Type *r, unsigned char *str, unsigned char *epos)
{
   int i;
   ...
   if (str == NULL)
	...
	r->beg_matches[0] = -1;
	r->end_matches[0] = 0;
	SLMEMSET(Closed_Paren_Matches, 0, sizeof(Closed_Paren_Matches));
     }
   else
     {
	r->beg_matches[0] = (int) (str - This_Str);
	r->end_matches[0] = (unsigned int) (epos - str);
     }

   for (i = 1; i < 10; i++)
     {
	if (Closed_Paren_Matches [i] == 0)
	  {
	     r->beg_matches[i] = -1;
	     r->end_matches[i] = 0;
	  }
				  ...
				  unsigned int len, SLRegexp_Type *reg)
{
   ...
   if (*buf == BOL)
     {
	if (NULL == (epos = regexp_looking_at(str, estr, buf + 1, cs)))
	  str = NULL;

	fixup_beg_end_matches (reg, str, epos);
	return str;
     }
   ...
   fixup_beg_end_matches (reg, NULL, epos);
   return NULL;
}
