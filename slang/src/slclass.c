...
int _SLang_push_datatype (unsigned char data_type)
{
    ...
    * int if they are the same size.  So, make sure we push the original
    * and not the copy.
    */
...
static int datatype_deref (unsigned char type, VOID_STAR ptr)
{
   ...
   return (*cl->cl_datatype_deref) (type);
}
...
void SLang_free_mmt (SLang_MMT_Type *ref)
{
   ...
   /* This can be zero if SLang_create_user_object is called followed
