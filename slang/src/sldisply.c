...
static int Can_Background_Color_Erase;
#endif
...
#define SLTT_USE_INSERT_HACK 1
   ...
   unsigned short insert_hack_prev = 0;
   unsigned short insert_hack_char = 0;
...
#if SLTT_HAS_NON_BCE_SUPPORT
   space_char = 0x20; /*  | (Bce_Color_Offset << 8); */
#else
   space_char = 0x20;
#endif
...
#  define TGETFLAG(x) (SLtt_tigetflag((x), &Tbuf) > 0)
# else
...
# endif
   ...
   if (Can_Background_Color_Erase == 0)
     Can_Background_Color_Erase = is_xterm || Linux_Console;
#endif
   get_color_info ();
