							 ...
							 double *xminptr, double *xmaxptr, double *dxptr)
{
   ...
   /* The result of this calculation is system dependent.  I simply do not
    * know what to do about it.  For example, under Linux, [0:0.9:0.1] results
    * in a 9 element array, whereas under solaris it produces a 10 element
    * array.  Sigh.
    */
...
static void array_map (void)
{
   ...
   unsigned int i;
   SLang_Name_Type *nt;
   unsigned int num_elements, increment;
   SLang_Array_Type *at;
   char *addr;
   unsigned char type;
   ...
   if (SLang_Error) SLang_push_null ();
}
