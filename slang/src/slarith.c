...
_INLINE_
static int
arith_typecast (unsigned char a_type, VOID_STAR ap, unsigned int na,
...
int SLang_pop_double(double *x, int *convertp, int *ip)
{
	...
	*x = (double) i;
	...
	/* All integers are store on stack as a long */
...
#else
     {NULL, SLANG_LONG_TYPE},
     {NULL, SLANG_ULONG_TYPE}
#endif
};
...
int _SLarith_register_types (void)
{
   ...
   if (-1 == SLclass_add_unary_op (SLANG_DOUBLE_TYPE, double_unary_op, arith_unary_op_result))
     return -1;
   _SLarith_Is_Arith_Type [SLANG_DOUBLE_TYPE] = 1;
   ...
   if (-1 == SLclass_add_unary_op (SLANG_FLOAT_TYPE, float_unary_op, arith_unary_op_result))
     return -1;
   _SLarith_Is_Arith_Type [SLANG_FLOAT_TYPE] = 1;
#endif
	...
	for (b_type = 0; b_type <= MAXIMUM_ARITH_TYPE_VALUE; b_type++)
	  {
	     ...
	     if (i != j)
	       if (-1 == SLclass_add_typecast (a_type, b_type, arith_typecast, implicit_ok))
		 return -1;
	  }
     }
