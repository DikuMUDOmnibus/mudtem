...
\usage{import (String_Type module)}
\description
  The \var{import} function causes the run-time linker to dynamically
  link to the specified shared object.  It seaches for the shared
  object as follows: First a search is performed along all module paths
  specified by the application.  Then a search is made along the paths
  defined via the \var{set_import_module_path} function.  If not
  found, a search is performed along the paths given by the 
  \var{SLANG_MODULE_PATH} environment variable.  Finally, a system
  dependent search is performed (e.g., using the \var{LD_LIBRARY_PATH}
...
\seealso{set_import_module_path, getenv, evalfile}
\done
